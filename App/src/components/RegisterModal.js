...
const RegisterModal = ({ isOpen, onClose, onRegisterSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        // Registration successful - now get the candidate ID by signing in
        const signInResult = await candidateAPI.signIn(formData.email);
        if (signInResult.success) {
          onRegisterSuccess('user', formData.email, signInResult.userId);
        } else {
          // If sign in fails, still call success but without userId
          onRegisterSuccess('user', formData.email, null);
        }
      } else {
